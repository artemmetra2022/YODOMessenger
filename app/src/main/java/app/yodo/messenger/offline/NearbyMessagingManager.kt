...
class NearbyMessagingManager @Inject constructor(
    ...
    private fun startAdvertising() {
        ...
        connectionsClient.startAdvertising(
            ...
            // Флаг остаётся false — повторный вызов startAdvertisingAndDiscovery сможет
            // попробовать снова (например, после выдачи разрешений).
            isAdvertising = false
        }
    }
    ...
    fun disconnect() {
        ...
        // После разрыва соединения — снова начать искать и быть видимым,
        // чтобы можно было подключиться к другому устройству без перезахода на экран.
        isAdvertising = false
        isDiscovering = false
        startAdvertising()
        startDiscovery()
    ...
    private val connectionLifecycleCallback = object : ConnectionLifecycleCallback() {
        ...
        override fun onConnectionResult(endpointId: String, resolution: ConnectionResolution) {
            if (resolution.status.statusCode == ConnectionsStatusCodes.STATUS_OK) {
                ...
                // Намеренно НЕ останавливаем advertising/discovery здесь.
                // Если их остановить сразу после соединения, первый пользователь
                // становится невидимым для тех, кто откроет экран позже.
                // При P2P_CLUSTER Nearby Connections сам управляет множеством соединений.
            } else {
                _connectionState.value = ConnectionState.DISCONNECTED
            }
        }
