...
fun DevicesScreen(
    ...
    Scaffold(
        ...
        Column(
            ...
            if (current != null) {
                SectionTitle(stringResource(R.string.devices_this_device))
                DeviceCard(
                    session = current,
                    isCurrent = true,
                    isTerminating = false,
            ...
            if (others.isNotEmpty()) {
                SectionTitle(stringResource(R.string.devices_other, others.size))
                ...
                OutlinedButton(
                    ...
                    Text(stringResource(R.string.devices_terminate_all))
                }
                ...
                Box(
                    ...
                    Text(
                        text = stringResource(R.string.devices_only_one),
                        style = MaterialTheme.typography.bodyMedium,
                        color = MaterialTheme.colorScheme.onSurfaceVariant
                    )
                }
    ...
    if (showTerminateAllDialog) {
        AlertDialog(
            ...
            title = { Text(stringResource(R.string.devices_terminate_title)) },
            text = {
                Text(stringResource(R.string.devices_terminate_text))
            },
            confirmButton = {
                Button(
                    onClick = {
                    ...
                    Text(stringResource(R.string.devices_terminate_btn))
                }
            },
            dismissButton = {
        ...
        if (!isCurrent) {
            ...
            AnimatedVisibility(visible = !isTerminating, enter = fadeIn(), exit = fadeOut()) {
                IconButton(onClick = onTerminate) {
                    Icon(
                        Icons.Filled.Close,
                        contentDescription = stringResource(R.string.devices_terminate_cd),
...
private fun formatLastActive(millis: Long): String {
    if (millis == 0L) return stringResource(R.string.devices_never)
    val now = System.currentTimeMillis()
    val diff = now - millis

    return when {
        diff < TimeUnit.MINUTES.toMillis(1)  -> stringResource(R.string.devices_just_now)
        ...
        diff < TimeUnit.DAYS.toMillis(2)     -> stringResource(R.string.devices_yesterday)
