...
fun ContactsScreen(
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text(stringResource(R.string.contacts_title), style = MaterialTheme.typography.titleLarge) },
                navigationIcon = {
        ...
        when (val state = uiState) {
            ...
            is ContactsUiState.NoPermission -> {
                Column(
                    ...
                    Button(onClick = { permissionLauncher.launch(Manifest.permission.READ_CONTACTS) }) {
                        Text(stringResource(R.string.contacts_permission_btn))
                    }
                }
            }
            is ContactsUiState.Empty -> {
                Box(modifier = Modifier.fillMaxSize().padding(padding)) {
                    Text(
                        stringResource(R.string.contacts_no_phone),
            ...
            is ContactsUiState.Content -> {
                LazyColumn(modifier = Modifier.fillMaxSize().padding(padding)) {
                    if (state.registered.isNotEmpty()) {
                        item {
                            Text(
                                stringResource(R.string.contacts_already_in_yodo),
                    ...
                    if (state.notRegistered.isNotEmpty()) {
                        item {
                            Text(
                                stringResource(R.string.contacts_invite),
                                style = MaterialTheme.typography.labelLarge,
                                modifier = Modifier.padding(horizontal = 16.dp, vertical = 8.dp)
                            )
                        }
                    ...
                    if (state.registered.isEmpty() && state.notRegistered.isEmpty()) {
                        item {
                            Text(
                                stringResource(R.string.contacts_not_found),
...
private fun NotRegisteredContactRow(contact: PhoneContact) {
    Row(
        ...
        Column {
            ...
            Text(
                stringResource(R.string.contacts_not_registered),
                style = MaterialTheme.typography.labelMedium,
                color = MaterialTheme.colorScheme.onSurfaceVariant
            )
        }
