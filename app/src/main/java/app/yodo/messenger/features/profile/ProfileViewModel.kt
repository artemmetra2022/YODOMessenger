...
class ProfileViewModel @Inject constructor(
    ...
    fun openSavedMessages() {
        viewModelScope.launch {
            try {
                val chatId = chatRepository.getOrCreateSavedChat()
                _savedChatId.value = chatId
            } catch (e: Exception) {
                _uiState.value = _uiState.value.copy(errorMessage = "Не удалось открыть Избранное")
            }
        }
    }

    fun consumeSavedChatId() { _savedChatId.value = null }
    ...
    // НОВОЕ: выход из аккаунта прямо с экрана профиля (как на макете)
