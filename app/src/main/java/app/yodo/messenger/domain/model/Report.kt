...
enum class ReportReason(val label: String) {
    ...
    FRAUD("Мошенничество"),
    // НОВОЕ (расширенная модерация): отдельные типы жалоб, по которым веб-админка
    // строит фильтр и диаграмму распределения. Хранится по имени (reason.name).
    NSFW("Неприемлемый контент (NSFW)"),
    ADVERTISING("Реклама"),
    OTHER("Другое"),
    // НОВОЕ (AD): обжалование глобальной блокировки аккаунта — падает в тот же раздел жалоб с пометкой «Обжалование».
