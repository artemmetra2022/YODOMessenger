...
function renderSupportInbox() {
  ...
  const conversations = Array.from(supportConversationsCache.values())
    .filter((conv) => chatMatchesSearch(conv, conv.supportUserName || ""))
    .sort((a, b) => {
...
/* ------------------------------------------------------------------ */
/* Локальные папки чатов (зеркало Android ChatFolder / DataStore)      */
/* ------------------------------------------------------------------ */

function chatFoldersStorageKey() {
  return currentUser ? `yodo_chat_folders_${currentUser.uid}` : "yodo_chat_folders_guest";
}

function loadChatFolders() {
  try {
    const raw = JSON.parse(localStorage.getItem(chatFoldersStorageKey()) || "[]");
    chatFolders = Array.isArray(raw)
      ? raw
          .filter((folder) => folder && typeof folder.id === "string" && typeof folder.name === "string")
          .map((folder, index) => ({
            id: folder.id,
            name: folder.name.trim().slice(0, 32) || `Папка ${index + 1}`,
            chatIds: Array.isArray(folder.chatIds) ? [...new Set(folder.chatIds.filter((id) => typeof id === "string"))] : [],
    ...
    console.warn("Папки: повреждённые локальные данные сброшены", error);
    chatFolders = [];
  }
...
function saveChatFolders() {
  chatFolders = chatFolders.map((folder, index) => ({ ...folder, order: index }));
  localStorage.setItem(chatFoldersStorageKey(), JSON.stringify(chatFolders));
}

function allFolderCandidateChats() {
  const unique = new Map();
  if (officialChannelData) unique.set(officialChannelData.id, officialChannelData);
  if (!isAdmin && supportChatData) unique.set(supportChatData.id, supportChatData);
  for (const chat of chatsCache.values()) {
    if (chat.type !== "SUPPORT") unique.set(chat.id, chat);
  }
  return Array.from(unique.values()).sort((a, b) => {
    const timeDiff = (b.lastMessageTimestamp || 0) - (a.lastMessageTimestamp || 0);
    return timeDiff || chatDisplayName(a).localeCompare(chatDisplayName(b), "ru");
  });
}

function activeFolder() {
  return activeFolderId === "all" ? null : chatFolders.find((folder) => folder.id === activeFolderId) || null;
}

function chatBelongsToActiveFolder(chat) {
  const folder = activeFolder();
  return !folder || folder.chatIds.includes(chat.id);
}
...
function renderFolderTabs() {
  const container = $("folder-tabs");
  if (!container) return;
  container.innerHTML = "";
  container.classList.toggle("hidden", viewingSupportInbox);
  if (viewingSupportInbox) return;

  const createTab = (id, label, count, editable = false) => {
    const tab = document.createElement("button");
    tab.type = "button";
    tab.className = "folder-tab" + (activeFolderId === id ? " active" : "");
    tab.setAttribute("aria-pressed", activeFolderId === id ? "true" : "false");
    tab.innerHTML = `<span>${esc(label)}</span>${count > 0 ? `<small>${count}</small>` : ""}`;
    ...
    if (editable) {
      ...
      manage.className = "folder-tab-manage";
      manage.textContent = "•••";
      manage.setAttribute("role", "button");
      manage.setAttribute("tabindex", "0");
      manage.setAttribute("aria-label", `Управлять папкой ${label}`);
      const openManage = (event) => { event.stopPropagation(); openFolderModal(id); };
      manage.addEventListener("click", openManage);
      manage.addEventListener("keydown", (event) => {
        if (event.key === "Enter" || event.key === " ") openManage(event);
      });
  ...
  createTab("all", "Все", allFolderCandidateChats().length);
  for (const folder of chatFolders) {
    const visibleCount = allFolderCandidateChats().filter((chat) => folder.chatIds.includes(chat.id)).length;
    createTab(folder.id, folder.name, visibleCount, true);
  }

  const add = document.createElement("button");
  add.type = "button";
  add.className = "folder-tab folder-tab-add";
  add.innerHTML = '<span aria-hidden="true">＋</span><span>Папка</span>';
  add.addEventListener("click", () => openFolderModal());
  container.appendChild(add);
}

function updateFolderSelectionCount() {
  const count = $("folder-chat-options").querySelectorAll('input[type="checkbox"]:checked').length;
  $("folder-chat-count").textContent = count === 1 ? "1 выбран" : `${count} выбрано`;
}

function renderFolderChatOptions(selectedChatIds = []) {
  const container = $("folder-chat-options");
  container.innerHTML = "";
  const chats = allFolderCandidateChats();
  if (chats.length === 0) {
    container.innerHTML = '<div class="folder-chat-empty">Сначала создайте чат — затем его можно будет добавить в папку.</div>';
    updateFolderSelectionCount();
    return;
  }
  for (const chat of chats) {
    const label = document.createElement("label");
    label.className = "folder-chat-option";
    const letter = (chatDisplayName(chat)[0] || "?").toUpperCase();
    label.innerHTML = `
      <input type="checkbox" value="${esc(chat.id)}" ${selectedChatIds.includes(chat.id) ? "checked" : ""}>
      <span class="chat-avatar">${esc(letter)}</span>
      <span class="folder-chat-copy"><strong>${esc(chatDisplayName(chat))}</strong><small>${esc(chat.lastMessage || "Нет сообщений")}</small></span>
      <span class="folder-checkmark" aria-hidden="true">✓</span>`;
    label.querySelector("input").addEventListener("change", updateFolderSelectionCount);
...
function openFolderModal(folderId = null) {
  ...
  const folder = folderId ? chatFolders.find((item) => item.id === folderId) : null;
  $("folder-modal-title").textContent = folder ? "Управление папкой" : "Новая папка";
  $("folder-name-input").value = folder?.name || "";
  $("folder-error").textContent = "";
  $("btn-save-folder").textContent = folder ? "Сохранить" : "Создать";
  $("btn-delete-folder").classList.toggle("hidden", !folder);
  renderFolderChatOptions(folder?.chatIds || []);
...
function closeFolderModal() {
  editingFolderId = null;
  $("modal-folder").classList.add("hidden");
}

$("btn-close-folder-modal").addEventListener("click", closeFolderModal);
$("btn-cancel-folder").addEventListener("click", closeFolderModal);
$("modal-folder").addEventListener("click", (event) => {
  if (event.target === $("modal-folder")) closeFolderModal();
});

$("btn-save-folder").addEventListener("click", () => {
  const name = $("folder-name-input").value.trim();
  const error = $("folder-error");
  if (!name) { error.textContent = "Введите название папки"; return; }
  const duplicate = chatFolders.some((folder) => folder.id !== editingFolderId && folder.name.toLocaleLowerCase("ru-RU") === name.toLocaleLowerCase("ru-RU"));
  if (duplicate) { error.textContent = "Папка с таким названием уже существует"; return; }
  const chatIds = Array.from($("folder-chat-options").querySelectorAll('input[type="checkbox"]:checked')).map((input) => input.value);

  const wasEditing = Boolean(editingFolderId);
  if (wasEditing) {
    const index = chatFolders.findIndex((folder) => folder.id === editingFolderId);
    if (index >= 0) chatFolders[index] = { ...chatFolders[index], name, chatIds };
  } else {
    const id = `folder_${Date.now().toString(36)}_${Math.random().toString(36).slice(2, 7)}`;
    chatFolders.push({ id, name, chatIds, isDefault: false, order: chatFolders.length });
    activeFolderId = id;
  }
  saveChatFolders();
  closeFolderModal();
  renderFolderTabs();
  renderChatList();
  showAppToast(wasEditing ? "Папка обновлена" : "Папка создана");
});

$("btn-delete-folder").addEventListener("click", () => {
  const folder = chatFolders.find((item) => item.id === editingFolderId);
  if (!folder || !window.confirm(`Удалить папку «${folder.name}»? Чаты останутся на месте.`)) return;
  chatFolders = chatFolders.filter((item) => item.id !== folder.id);
  if (activeFolderId === folder.id) activeFolderId = "all";
  saveChatFolders();
  closeFolderModal();
  renderFolderTabs();
  renderChatList();
  showAppToast("Папка удалена");
});

function chatMatchesSearch(chat, displayName = "") {
  ...
  return searchable.includes(chatListSearchQuery);
}

function renderChatSearchEmpty(listEl) {
  const empty = document.createElement("div");
  empty.className = "chat-search-empty";
  empty.innerHTML = '<span aria-hidden="true">⌕</span><strong>Ничего не найдено</strong><small>Попробуйте изменить запрос</small>';
  listEl.appendChild(empty);
}
...
function renderChatList() {
  ...
  const chats = Array.from(chatsCache.values())
    ...
    .filter((c) => chatBelongsToActiveFolder(c))
    .filter((c) => chatMatchesSearch(c))
    .sort((a, b) => (b.lastMessageTimestamp || 0) - (a.lastMessageTimestamp || 0));

  listEl.innerHTML = "";

  // Официальный канал — всегда сверху, если существует.
  if (officialChannelData && chatBelongsToActiveFolder(officialChannelData) && chatMatchesSearch(officialChannelData)) {
    listEl.appendChild(buildChatListItem(officialChannelData));
  }

  // Собственная беседа поддержки (для обычных пользователей).
  if (!isAdmin && supportChatData && chatBelongsToActiveFolder(supportChatData) && chatMatchesSearch(supportChatData)) {
  ...
  if (!listEl.querySelector(".chat-item")) {
    if (chatListSearchQuery) renderChatSearchEmpty(listEl);
    else if (activeFolder()) {
      const empty = document.createElement("div");
      empty.className = "chat-search-empty folder-empty-state";
      empty.innerHTML = '<span aria-hidden="true">📁</span><strong>Папка пока пуста</strong><small>Откройте меню папки и добавьте чаты</small>';
      listEl.appendChild(empty);
    }
  }
  renderFolderTabs();
...
const chatListSearchInput = $("chat-list-search");
const clearChatSearchButton = $("btn-clear-chat-search");

function setChatListSearch(value) {
  chatListSearchQuery = value.trim().toLocaleLowerCase("ru-RU");
  clearChatSearchButton.classList.toggle("hidden", !chatListSearchQuery);
  document.querySelector(".sidebar-search-shortcut")?.classList.toggle("hidden", !!chatListSearchQuery);
  if (viewingSupportInbox) renderSupportInbox();
  else renderChatList();
}

chatListSearchInput.addEventListener("input", (event) => setChatListSearch(event.target.value));
clearChatSearchButton.addEventListener("click", () => {
...
document.addEventListener("keydown", (event) => {
  ...
  if (event.key === "Escape" && document.activeElement === chatListSearchInput && chatListSearchInput.value) {
    chatListSearchInput.value = "";
    setChatListSearch("");
  }
});

$("btn-empty-new-chat").addEventListener("click", () => $("btn-new-chat").click());
